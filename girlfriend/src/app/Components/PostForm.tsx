...
export default function PostForm({ newPost, setNewPost, handleAddImage, handleDeleteImage, handleSavePost }: Props) {
  return (
    <div className="bg-pink-100/60 p-4 rounded-lg shadow-lg mb-6 border border-pink-200">
      <input
        type="text"
        placeholder="Título"
        value={newPost.title}
      ...
      <textarea
        ...
        className="w-full mb-3 p-2 rounded-lg border border-pink-300 bg-pink-100 text-gray-700 focus:outline-none focus:ring-2 focus:ring-pink-400"
      />
      <label className="flex justify-center items-center mb-3 w-full font-medium bg-pink-400 hover:bg-pink-500 text-white px-4 py-2 rounded-lg cursor-pointer transition text-center">
        Adicionar Fotos
